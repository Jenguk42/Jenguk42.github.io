...
5. Adding react icons: `npm i react-icons`
...
## States: Creating a Hook with `useState`
...
The function for deleting a task should be within `App.js`, and the function itself should be passed into `Task.js` and `Tasks.js` as props.

```react
// In App.js, add a function to delete Task
const deleteTask = (id) => {
    setTasks(tasks.filter((task) => task.id !== id))
}

// When calling tasks in App.js, add a prop onDelete
<Tasks tasks={tasks} onDelete={deleteTask}/>

// Tasks.js sends the prop to Task.js
<Task key={task.id} task={task} onDelete={onDelete}/>

// The function can be called when the x button in Task.js is clicked
<FaTimes 
